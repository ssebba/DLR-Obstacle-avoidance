...
class lidar_filter(Node):
    ...
    def __init__(self):
        ...
        self.publisher_scans = self.create_publisher(Float32MultiArray, '/lidar_data', 1)
        self.publisher_angles = self.create_publisher(Float32MultiArray, '/lidar_angles', 1)         
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        idx = np.linspace(
            0,              # first index of the sector
            total_ranges - 1, # last index of the sector
            self.num_lidar_ranges, # number of sectors
            dtype=int           # type of the index
            )
        
        filtered_ranges = ranges[idx]   # assign values for each index found
        filtered_angles = lidar_scan.angle_min + idx * lidar_scan.angle_increment # find angles of each index 

        out_data.data = [float(x) for x in filtered_ranges]
        out_angles.data = [float(x) for x in filtered_angles]
        if self.step_count % 500 == 0:
            self.step_count = 0
            self.get_logger().info('I am publishing!')

        self.publisher_scans.publish(out_data)
        self.publisher_angles.publish(out_angles)
        self.step_count += 1
